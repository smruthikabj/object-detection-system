...
import numpy as np
import cv2
from object_detection.utils import label_map_util  # Utility to load label map
...
def upload_image():
    ...
    detected_items = []
    for i in range(len(detections_score)):
        if detection_classes[i]>=0.5:
             item = category_index[detection_classes][i]['name']
             score = detections[i]
    ...
    return render_template('results.html',detected_items=detected_items,image_data=img_io.getvalue().decode('latin1'))
